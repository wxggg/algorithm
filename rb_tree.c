...
#ifdef DEBUG
static inline unsigned long long node_id(struct rb_node *node)
{
...
unsigned long long rb_node_key_start(struct rb_node *node)
{
	return node->start;
}

unsigned long long rb_node_key_end(struct rb_node *node)
{
	return node->end;
}
...
static void tree_dump(struct rb_node *node, uint32_t level)
{
	...
	printf("%.*s%c----(%lld,%c) <%lld,%lld,%lld> [0x%llx,0x%llx]",
	       (int)level * 4,
	       "                                                      ",
...
static int tree_validate(struct rb_node *root, struct rb_node **fail_node)
{
	...
	if (left != right) {
		printf("node-%lld is not valid, left:%d != right:%d",
		        node_id(root), left, right);
		*fail_node = root;
		return -EINVAL;
	}
...
static void rb_tree_validate(struct rb_tree *tree)
{
	...
	if (tree_validate(tree->root, &fail_node) < 0)
	{
		...
		printf("node-%lld failed\n", node_id(fail_node));
		assert(0);
	}
}
#endif

struct rb_node * rb_tree_search(struct rb_tree *tree, uintptr_t key)
{
...
struct rb_node *
rb_tree_insert(struct rb_tree *tree, uintptr_t start, uintptr_t end, void *value)
{
	...
	while (node) {
		...
		if (start > node->end) {
			...
			printf("insert [%p, %p] overlaps with node [%p, %p]",
				(void *)start, (void *)end,
...
int rb_tree_remove(struct rb_tree *tree, uintptr_t key)
{
...
struct rb_tree * rb_tree_create(void)
{
	...
	tree = (struct rb_tree *) malloc(sizeof(*tree));
	if (!tree)
		return NULL;
...
int main(int argc, char **argv)
{
	...
	switch(argc) {
		case 3:
			...
			break;
		default:
			printf("usage: ./rb_tree -n num\n");
			exit(-1);
	}

	for (i = 0; i < max_nodes; i++) {
		node = rb_tree_insert(tree, i *100 , i * 100 + 30, NULL);
		assert(node);
	}

	for (i = 0; i < max_nodes; i++) {
		ret = rb_tree_remove(tree, i * 100 + 10);
		assert(!ret);
	}
